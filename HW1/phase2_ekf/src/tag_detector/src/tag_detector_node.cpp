...
void process(const std::vector<cv::Point3f> &pts_3, const std::vector<cv::Point2f> &pts_2, const ros::Time& frame_time)
{
    ...
    //cout<<"corordinate: "<< t.at<double>(0, 0)<<' '<<t.at<double>(1, 0)<<' '<<t.at<double>(2, 0)<<endl;
    ...
    int random_choice = pts_3.size();  //随机选取几个点.这是一个超参数
    int iteration_num = 1; //设置ransac的迭代轮数
    double ransac_threshold = 0.01f;  //设置ransac的阈值
    int cnt = 0;
    int max_correct_point = INT_MIN;
    double min_erro = INT_MAX;
    Matrix3d tmpR;
    Vector3d tmpT;
    ...
    while(((cnt++) < iteration_num)){
        ...
        EquMatrix.setZero();
        //初始化该矩阵，根据推导的表达式
       // vector<bool> visit(pts_3.size(),false);
        for(int i = 0;i < pts_3.size();i++)
        {
            //int choice_index = rand()%pts_3.size();
            //if(visit[choice_index]==true) continue;
            //visit[choice_index] = true;
            EquMatrix(2*i,0) = -double(pts_3[i].x);
            EquMatrix(2*i,1) = -double(pts_3[i].y);
            EquMatrix(2*i,2) = -1.0;
            EquMatrix(2*i,6) = double(new_point[i].x)*double(pts_3[i].x);
            EquMatrix(2*i,7) = double(new_point[i].x)*double(pts_3[i].y);
            EquMatrix(2*i,8) = double(new_point[i].x);
            //设置下一行的参数
            EquMatrix(2*i+1,3) = -double(pts_3[i].x);
            EquMatrix(2*i+1,4) = -double(pts_3[i].y);
            EquMatrix(2*i+1,5) = -1.0;
            EquMatrix(2*i+1,6) = double(new_point[i].y)*double(pts_3[i].x);
            EquMatrix(2*i+1,7) = double(new_point[i].y)*double(pts_3[i].y);
            EquMatrix(2*i+1,8) = double(new_point[i].y);
        }

        //求解方程组
        //利用SVD求解最小二乘
        JacobiSVD<MatrixXd> svd(EquMatrix, ComputeFullV);
        MatrixXd answer1 = svd.matrixV().rightCols(1);
        Matrix3d answer;
        Matrix3d answer_3d;
        ...
        tmpT = answer.col(2)/sqrt(answer.col(1).norm()*answer.col(0).norm());
        //判断正负符号choice_index
        //这一项需要保证点在相机的前方
        int pos = 0;
        int neg = 0;
    ...
    if(pre_R.isZero()){
        ...
        R = _LOW_PASS_PARA*R+(1-_LOW_PASS_PARA)*pre_R;
        pre_R=R;
    }
    Quaterniond Q_yourwork;
    Q_yourwork = R;
    double norm_v = sqrt(Q_yourwork.x()*Q_yourwork.x()+Q_yourwork.y()*Q_yourwork.y()
                    +Q_yourwork.z()*Q_yourwork.z()+Q_yourwork.w()*Q_yourwork.w());
    ...
    odom_yourwork.pose.pose.orientation.w = Q_yourwork.w()/norm_v;
    odom_yourwork.pose.pose.orientation.x = Q_yourwork.x()/norm_v;
    odom_yourwork.pose.pose.orientation.y = Q_yourwork.y()/norm_v;
    odom_yourwork.pose.pose.orientation.z = Q_yourwork.z()/norm_v;
